...
export default function LessonShell({
  ...
  const sidebar = (
    <Card className="h-full rounded-2xl border border-app bg-card p-3 shadow-sm backdrop-blur-sm">
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        active && "bg-[rgba(255,255,255,0.08)] shadow-inner"
                      )}
                    >
                      {status === "done" ? (
                        <CircleCheckBig className="size-4 text-emerald-500" />
                      ) : (
                        <div className="size-4 rounded-full border-2 border-app group-hover:border-white/20 " />
                      )}
    ...
    <>
      {cmdOpen ? (
        ...
        >
          ...
          >
            <Card className="overflow-hidden rounded-2xl border border-app bg-card p-0 shadow-2xl backdrop-blur-sm">
              <Command className="w-full">
                ...
                <Command.List className="max-h-[420px] overflow-auto p-2">
                  <Command.Empty className="p-4 text-sm text-secondary">
                    Ничего не найдено.
                  </Command.Empty>
                      ...
                      {m.lessons.map((l) => (
                        <Command.Item
                          ...
                          className={cn(
                            ...
                            "aria-selected:bg-[rgba(255,255,255,0.06)]"
                          )}
                        >
      ...
      <div className="grid h-[calc(100vh-64px)] gap-4 lg:grid-cols-[auto_12px_1fr] overflow-hidden">
        ...
        <main className="h-full overflow-y-auto p-4 md:p-6 space-y-6">
          ...
          <Card className="rounded-2xl border border-app bg-card p-6 shadow-sm backdrop-blur-sm">
          ...
          {/* THEORY */}
          {step === "theory" ? (
            <Card className="rounded-2xl border border-app bg-card p-8 shadow-sm backdrop-blur-sm">
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "bg-soft text-secondary hover:bg-[rgba(255,255,255,0.04)]",
        active && "bg-[rgba(255,255,255,0.08)] text-primary shadow-md",
        locked && "cursor-not-allowed opacity-50 hover:bg-soft"
      )}
    >
      {icon}
      {title}
      {done ? <CircleCheckBig className="size-4 text-emerald-500" /> : null}
      {locked ? <Lock className="size-4 text-secondary" /> : null}
    </button>
  );
}
...
function BlockRenderer({ block }: { block: any }) {
  switch (block.type) {
    case "heading":
      return (
        <h2 className="mt-6 text-2xl font-semibold tracking-tight text-primary first:mt-0">
          {block.text}
        </h2>
      );
    case "paragraph":
      return (
        <p className="whitespace-pre-line text-base leading-7 text-secondary">
          {block.text}
        </p>
      );
    case "video":
    ...
    case "vocab":
      return (
        <div className="rounded-2xl border border-app bg-card p-6 shadow-sm backdrop-blur-sm">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            {block.items?.map((it: any, i: number) => (
              <div
                key={i}
                className="rounded-2xl border border-app bg-soft p-5 shadow-sm"
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-semibold text-primary">{it.word}</span>
